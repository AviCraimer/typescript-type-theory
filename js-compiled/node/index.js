...
var TBD04_lambda_js_functions_exports = {};
module.exports = __toCommonJS(TBD04_lambda_js_functions_exports);
...
function shift(offset, t, minIndex = 0, maxIndex = Infinity) {
  ...
  switch (t.syntax) {
    case "Var":
      return t.index >= minIndex && t.index <= maxIndex ? { ...t, index: t.index + offset } : t;
    case "Abs":
      return {
        ...t,
...
function mkAbs(body, indexInCtx) {
  var _a, _b;
  const { context, term } = body;
  const paramType = (_a = context[indexInCtx]) == null ? void 0 : _a.type;
  if (paramType === void 0) {
  ...
  const newTerm = {
    ...
    body: shiftedTerm,
    paramName: ((_b = context[indexInCtx]) == null ? void 0 : _b.name) ?? ""
  };
  return {
    context: newContext,
    term: newTerm
...
function mkApp(func, arg) {
  ...
  if (!(0, import_lodash.isEqual)(domainType, argTerm.type)) {
    throw new Error("Type mismatch between function domain and argument");
  }
  const matchingContexts = (0, import_lodash.isEqual)(funcCtx, argCtx);
  let newArg = argTerm;
  let newCtx = argCtx;
  if (!matchingContexts) {
...
var toStringDeBruijin = (term) => {
  const inner = (term2) => {
    if (term2.syntax === "Var") {
      return String(term2.index);
    } else if (term2.syntax === "Abs") {
      return `[\u03BB${inner(term2.body)}]`;
    } else {
