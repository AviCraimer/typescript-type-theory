(() => {
  ...
  // episodes/e02-lambda-reduction/lambdaExpressions.ts
  function isBound(x2) {
    if (typeof x2 === "string") {
      return x2.indexOf("\u02DA") === 0;
    } else {
  ...
  // episodes/e02-lambda-reduction/index.ts
  var import_lodash = __toModule(require_lodash());
  console.log("\n".repeat(15));
  var alpha = (abs, boundVar) => {
    return __spreadProps(__spreadValues({}, abs), {
      parameter: boundVar,
      body: substitution(abs.body, abs.parameter, boundVar)
    });
  };
  var isRedex = (expression) => {
    if (typeof expression !== "string" && expression.role === "Application" && typeof expression.func !== "string" && expression.func.role === "Abstraction") {
      return true;
    } else {
      return false;
    }
  };
  var beta1 = (expression) => {
    if (isRedex(expression)) {
      const redex2 = expression;
      const abs = redex2.func;
      return substitution(abs.body, abs.parameter, redex2.argument);
    }
    return expression;
  };
  var x = "x";
  ...
  var w = "w";
  var f = "f";
  var abs1 = abstract(f, abstract(x, apply(f, x, y)));
  var redex = apply(abs1, z);
  ...
  var badSubstitution = beta1(apply(abs1, abstract(x, x)));
  console.log(apply(badSubstitution, z).toString());
  console.log(beta1(apply(badSubstitution, z)).toString());
  var getUniqueParamNames = (expression) => {
    const getParamsInExpression = (expression2) => {
      if (typeof expression2 === "string") {
        return isBound(expression2) ? [expression2] : [];
      } else if (expression2.role === "Application") {
        const { func, argument } = expression2;
        return [
          ...getParamsInExpression(func),
          ...getParamsInExpression(argument)
        ];
      } else if (expression2.role === "Abstraction") {
        const { parameter, body } = expression2;
        return [parameter, ...getParamsInExpression(body)];
      }
      let nothing = expression2;
      return expression2;
    };
  ...
  var beta = (() => {
    ...
    return (expression) => {
      if (isRedex(expression)) {
        const redex2 = expression;
        const { func, argument } = redex2;
        const { parameter, body } = func;
        const abstractionParameters = getUniqueParamNames(func);
        let newArg = argument;
        abstractionParameters.forEach((parameter2) => {
          newArg = substitution(newArg, parameter2, `${parameter2}_${count()}`);
        });
  ...
  var abs1Substitution = beta(apply(abs1, abstract(x, x)));
  ...
  var subExps = (expression, order = "post") => {
    const subExps2 = [];
    const step = (expression2, parent = void 0, relationship = "root") => {
      if (order === "pre") {
        subExps2.push({ parent, relationship, subExp: expression2 });
      }
      if (typeof expression2 === "object") {
        if (expression2.role === "Application") {
          step(expression2.func, expression2, "func");
          step(expression2.argument, expression2, "argument");
        }
        if (expression2.role === "Abstraction") {
          step(expression2.body, expression2, "body");
        }
      }
      if (order === "post") {
        subExps2.push({ parent, relationship, subExp: expression2 });
  ...
  console.log("\nSub Expressions");
  var app1 = apply(apply(abstract(x, x), z), w);
  print(...subExps(app1).map((x2) => {
    const { parent, subExp, relationship } = x2;
    return `${x2.subExp.toString()}  <--${relationship}--  ${x2.parent ? x2.parent.toString() : "undefined"}`;
  }));
  var normalizeApplicative = (expression, log = false, maxSteps = 30) => {
    const step = (expression2) => {
      expression2 = (0, import_lodash.cloneDeep)(expression2);
      const subExpArray = subExps(expression2, "post");
      const redexArray = subExpArray.filter(({ subExp }) => isRedex(subExp));
      if (redexArray.length === 0) {
        return expression2;
      }
      const { parent, subExp: redex2, relationship } = redexArray[0];
      const reduced = beta(redex2);
      if (!parent && relationship === "root") {
        return reduced;
      } else if (parent) {
        if (parent.role === "Abstraction") {
          parent.body = reduced;
        } else if (parent.role === "Application" && relationship !== "root" && relationship !== "body") {
    ...
    let prev = "___EMPTY___";
    while (counter < maxSteps && !(0, import_lodash.isEqual)(current, prev)) {
  ...
  console.log("\nNormalize");
  var exp2 = apply(abstract(x, apply(x, y)), apply(abstract(z, z), f));
