...
(() => {
  ...
  // episodes/e02a-substitution-references/lambdaExpressions.ts
  ...
  function boundVar(name, binder, parent) {
    return {
      syntax: "Variable",
      name,
      children: emptyArray,
  ...
  var combineFreeVars = (fv1, fv2) => {
    const combined = {};
    const varNames = [.../* @__PURE__ */ new Set([...Object.keys(fv1), ...Object.keys(fv2)])];
    varNames.forEach((name) => {
      var _a, _b;
      const vars1 = (_a = fv1[name]) != null ? _a : [];
      const vars2 = (_b = fv2[name]) != null ? _b : [];
      const vars = [.../* @__PURE__ */ new Set([...vars1, ...vars2])];
      combined[name] = vars;
    });
    return combined;
  };
  function applyOnce(func, arg) {
    const funcExp = (0, import_lodash.cloneDeep)(func);
    const argExp = (0, import_lodash.cloneDeep)(arg);
    const app = {
      syntax: "Application",
  ...
  var abstract = (abstractOver, ...vars) => {
    var _a;
    const [variableName, ...rest] = vars;
    const body = (0, import_lodash.cloneDeep)(abstractOver);
    const toSubstitute = (_a = body.freeVars[variableName]) != null ? _a : [];
  ...
  var print = (...toPrint) => {
    toPrint.forEach((x2) => {
      if (typeof x2 === "string") {
        ...
        console.log(toString(x2));
      }
    });
  };
  var reduceRedex = (redex) => {
    var _a;
    const [abstraction, argument] = redex.children;
    const { parent } = redex;
    const absCopy = (0, import_lodash.cloneDeep)(abstraction);
    const argCopies = [];
    absCopy.boundVarInstances.forEach((bVar) => {
      const argCopy = (0, import_lodash.cloneDeep)(argument);
      if ("binder" in argument && "binder" in argCopy) {
        argCopy.binder = argument.binder;
      }
      argCopy.parent = bVar.parent;
    ...
    if (argument.syntax === "Variable" && "binder" in argument) {
      ...
      console.log("argCopies", (_a = argCopies[0].parent) == null ? void 0 : _a.parentExp.syntax);
    ...
    if (parent) {
      ...
      newExpression.parent = void 0;
    }
    return newExpression;
  };

  // episodes/e02a-substitution-references/index.ts
  var x = Var("x");
  var y = Var("y");
  var z = Var("z");
  var w = Var("w");
  ...
  var g = Var("g");
  var h = Var("h");
  var example1 = abstract(apply(x, y, apply(z, x)), "x", "y");
  print(example1);
  var afterSub1 = reduceRedex(apply(example1, apply(g, h)));
  console.log("after sub");
  print(example1, afterSub1);
  var afterSub2 = reduceRedex(apply(afterSub1, apply(f, x)));
  print(afterSub2);
  var innerLambdaApplication = abstract(apply(abstract(x, "x"), f), "f");
  reduceRedex(innerLambdaApplication.children[0]);
  print(innerLambdaApplication);
  console.log(
    innerLambdaApplication.boundVarInstances[0] === innerLambdaApplication.children[0]
  );
})();
