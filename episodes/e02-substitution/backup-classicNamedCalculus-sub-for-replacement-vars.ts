...
> = {
    ...
    abstraction: (abs, inner, replacementExpr, varToReplace) => {
        ...
        let freshReplacement: LambdaExpr = replacementExpr;
        if (varEq(boundVar, varToReplace)) {
            ...
            console.log("getting fresh variable in replacement");
            //Get a fresh variable that is not free in the replacement expression
            const freshVar = getFreshVar(
                replacementChildFreeVars,
                boundVar.name
            );

            freshReplacement = inner(replacementExpr, freshVar, boundVar);
        }
        ...
        const newBody = inner(body, freshReplacement, varToReplace);

        return abstraction(abs.boundVar, newBody);
    },
